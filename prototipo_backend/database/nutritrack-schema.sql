...
-- Canonical PostgreSQL baseline for NutriTrack.
-- Design goals:
-- 1. Keep core domain entities relational and strongly constrained.
-- 2. Use JSONB only for provider payloads/metadata that are genuinely flexible.
-- 3. Model device data as timestamped observations linked to a device connection.

CREATE TYPE account_status_t AS ENUM ('active', 'inactive', 'pending', 'suspended');
CREATE TYPE recipe_source_type_t AS ENUM ('manual', 'assistant', 'imported');
...
CREATE TABLE user_profiles (
    ...
    include_burned_calories_in_goal BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE progress_logs (
    ...
    physical_activities JSONB NOT NULL DEFAULT '[]'::jsonb,
    sleep_hours NUMERIC(4,2),
    note TEXT,
    source_type measurement_source_t NOT NULL DEFAULT 'manual',
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE device_connection_permissions (
    id BIGSERIAL PRIMARY KEY,
    device_connection_id BIGINT NOT NULL REFERENCES device_connections(id) ON DELETE CASCADE,
    permission_key VARCHAR(80) NOT NULL,
    is_granted BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT device_connection_permissions_uniq UNIQUE (device_connection_id, permission_key)
);
...
CREATE TABLE device_sync_runs (
    ...
    sync_status sync_status_t NOT NULL DEFAULT 'running',
...
CREATE TABLE device_measurements (
    ...
    measurement_source measurement_source_t NOT NULL DEFAULT 'device',
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT device_measurements_value_chk CHECK (numeric_value IS NOT NULL OR text_value IS NOT NULL)
);

CREATE TABLE openfoodfacts_products_cache (
...
CREATE INDEX idx_device_measurements_user_type ON device_measurements(user_id, measurement_type, measured_at DESC);
CREATE INDEX idx_device_measurements_connection_time ON device_measurements(device_connection_id, measured_at DESC);
